...
const colorMap = {
  bronze: { bar: "bg-bronze-400", label: "text-[#9E8A26]", border: "border-bronze-400/20", bg: "bg-bronze-400/5" },
                    ...
                    <div key={i}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-500 text-[10px] font-mono">{item.role}</span>
                        </div>
                        <span className={`text-[10px] font-mono ${c.label}`}>{item.level}%</span>
                      </div>
                      <div className="h-0.5 bg-obsidian-700/80 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
        ...
        >
          ...
          <div className="flex items-center gap-3 shrink-0">
            {["Firebase", "AWS", "Vercel"].map((t, i) => (
              <div key={i} className="px-3 py-1.5 rounded-sm border border-gray-800 text-xs text-gray-400 font-mono">
                {t}
              </div>
            ))}
          </div>
