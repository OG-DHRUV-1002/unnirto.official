...
const ventures = [
  {
    ...
    resolution:
      "Digitized 100% of workflows across 3 clients. Handles 150+ daily patients with 0% downtime and 100% automated billing accuracy. Deployed in a 60-day Velocity Sprint.",
    metrics: [
      { label: "Daily Patients", value: "150+" },
      { label: "Downtime", value: "0%" },
      { label: "Billing Accuracy", value: "100%" },
      { label: "Sprint Duration", value: "60 Days" },
    ],
    clients: [
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-black text-white leading-tight mb-4">
            Portfolio
            <span className="text-bronze-400"> Deployments</span>
          </h2>
        ...
        >
          ...
          <div className="border-b border-bronze-400/10 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="text-xs font-mono text-bronze-400 bg-bronze-400/10 px-2 py-0.5 rounded-full border border-bronze-400/20">
                  {venture.label}
                </span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 border-b border-bronze-400/10">
            {venture.metrics.map((m, i) => (
              ...
              >
                <div className="text-3xl font-black text-bronze-400">
                  {m.value}
                </div>
              ...
              (tab) => (
                <button
                  ...
                  className={`px-6 py-3 text-xs font-mono uppercase tracking-widest transition-all duration-200 ${
                    activeTab === tab
                      ? "text-bronze-400 border-b-2 border-bronze-400 bg-bronze-400/5"
          ...
          <div className="p-6">
            <div className="flex items-start gap-3">
              ...
              {activeTab === "architecture" && (
                <>
                  <Zap className="text-bronze-400 mt-0.5 shrink-0" size={18} />
          ...
          <div className="px-6 pb-6 flex flex-wrap gap-2 border-t border-bronze-400/10 pt-4">
            <span className="text-xs text-gray-600 font-mono mr-2 self-center">
              STACK:
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {upcomingVentures.map((v, i) => (
            <motion.div
              ...
              className="glass-card rounded-sm p-6 border border-gray-700/30 hover:border-bronze-400/20 transition-all duration-300 group"
            >
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-mono text-gray-600 bg-obsidian-700/50 px-2 py-0.5 rounded-full border border-gray-700/40 uppercase tracking-wider">
                  {v.stage}
                </span>
                <ArrowUpRight
                  size={14}
                  className="text-gray-700 group-hover:text-bronze-400 transition-colors"
                />
              </div>
              ...
              <div className="flex flex-wrap gap-1">
                {v.stack.map((s, j) => (
                  <span
                    key={j}
                    className="text-[10px] px-2 py-0.5 rounded font-mono bg-obsidian-700/30 border border-gray-700/30 text-gray-600"
                  >
                    {s}
                  </span>
                ))}
